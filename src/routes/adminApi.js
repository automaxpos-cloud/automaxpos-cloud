  ...
  router.get("/licenses", adminJwt, async (_req, res) => {
    try {
      ...
      SELECT
        bl.id,
        bl.license_id,
        ${selectCol(cols, "plan", "plan")},
        ${selectCol(cols, "device_limit", "device_limit")},
        ${selectCol(cols, "plan_name", "plan_name")},
        ${selectCol(cols, "base_device_limit", "base_device_limit")},
        ${selectCol(cols, "extra_device_count", "extra_device_count")},
        ${selectCol(cols, "total_device_limit", "total_device_limit")},
        ${selectCol(cols, "license_version", "license_version")},
        ${selectCol(cols, "previous_license_id", "previous_license_id")},
        ${selectCol(cols, "change_reason", "change_reason")},
        ${selectCol(cols, "license_status", "license_status")},
        ${selectCol(cols, "request_id", "request_id")},
        ${selectCol(cols, "hardware_bundle", "hardware_bundle")},
        ${selectCol(cols, "quoted_price", "quoted_price")},
        0 AS used_devices,
        ${selectCol(cols, "issued_at", "issued_at")},
        ${selectCol(cols, "expires_at", "expires_at")},
        ${selectCol(cols, "grace_ends_at", "grace_ends_at")},
        COALESCE(${selectCol(cols, "status")}, 'ACTIVE') AS status,
